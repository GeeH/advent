...
class Santa
{
    ...
    private $roboSanta = false;
    private $santasTurn = true;
    private $roboSantaLocation = [0, 0];

    public function deliverDirections(string $directions)
    {
        $directions = str_split($directions);
        foreach ($directions as $count => $direction) {
            $this->moveLocation($direction);
            if ($this->roboSanta) {
                $this->santasTurn = !$this->santasTurn;
            }
        }
    ...
    public function resetSanta()
    {
        $this->map = ['0x0' => 1];
        $this->location = [0, 0];
        $this->roboSantaLocation = [0, 0];
        $this->roboSanta = false;
        $this->santasTurn = true;
    }

    public function moveLocation(string $direction)
    {
        if ($direction === '>') {
            $this->moveEast();
        }
    ...
    private function incrementDelivery()
    {
        if ($this->santasTurn) {
            $this->map[$this->location[0] . 'x' . $this->location[1]]++;
            return;
        }
        $this->map[$this->roboSantaLocation[0] . 'x' . $this->roboSantaLocation[1]]++;
    }
    ...
    public function enableRoboSanta()
    {
        $this->roboSanta = true;
        $this->map['0x0']++;
    }
 ...
 */
$santa = new Santa();
assert($santa->deliverDirections('>') === 2);
$santa->resetSanta();
assert($santa->deliverDirections('^>v<') === 4);
$santa->resetSanta();
assert($santa->deliverDirections('^v^v^v^v^v') === 2);
$santa->resetSanta();

$santa->enableRoboSanta();
assert($santa->deliverDirections('^v') === 3);
$santa->resetSanta();
$santa->enableRoboSanta();
assert($santa->deliverDirections('^>v<') === 3);
$santa->resetSanta();
$santa->enableRoboSanta();
assert($santa->deliverDirections('^v^v^v^v^v') === 11);
$santa->resetSanta();
...
echo $santa->deliverDirections($directions) . PHP_EOL;
